...
description: Manage live sandbox sessions with ps, attach, detach, stop, inspect, and session cleanup
---
...
| `nono session cleanup` | Remove old session records from the local registry |
...
Exited sessions stay in the registry until you remove them. Use `nono session cleanup` to clean them up:

```bash
nono session cleanup
```

Common variants:

```bash
nono session cleanup --dry-run
nono session cleanup --older-than 7
nono session cleanup --keep 20
```

`nono session cleanup` currently skips sessions in `running` state and removes exited sessions. `paused` sessions are treated more conservatively elsewhere in the runtime than they are here, so do not use it as a maintenance step for paused/stopped sessions until that behavior is tightened.
