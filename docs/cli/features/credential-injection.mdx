...
If `command[0]` is a bare name (e.g. `"gh"`, not `/usr/local/bin/gh`), it's resolved by walking `PATH` — but only through directories the sandbox does not have write access to. This matters because this resolution runs host-side, unsandboxed: a bare-name lookup that considered every `PATH` directory would let a sandboxed process plant a same-named binary in any directory it can write to (a workdir, a cache dir under `filesystem.allow`) and have it run with real user privileges instead of the intended command. See [Security Model](/cli/internals/security-model#path-sanitization-for-host-side-brokers) for the full mechanism. Use an absolute path in `command[0]` to skip this resolution entirely.
...
Profiles can declare which credentials to load in the `env_credentials` section:
