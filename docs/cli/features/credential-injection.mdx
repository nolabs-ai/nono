...
```bash
# 1. Store the credential (prompts for the secret, no echo)
nono credential set openai
...
`nono credential` stores and inspects credentials under the exact account each
service reads at run time, so a keystore entry cannot end up somewhere the
proxy never looks.

```bash
nono credential set openai          # prompt for the secret (no echo), store it
nono credential list                # every service, its source, and availability
nono credential check openai        # verify the credential loads (never prints it)
nono credential rm openai           # remove a stored credential
```

`list` shows where each service's credential comes from:
...
Pass `--profile <name>` to include the profile's own `custom_credentials`, and
`--json` for machine-readable output. Sources backed by an external manager
(1Password, Bitwarden, Apple Passwords) report `unknown` rather than running
that manager's CLI; `nono credential check <service>` asks it for a real answer.
A route that authenticates without a stored secret — AWS SigV4, SPIFFE, an
OAuth2 client assertion — reports `unknown` for the same reason: there is
nothing to look up.

A locked keystore makes every lookup wait out `NONO_KEYRING_TIMEOUT_SECS`
(120 s by default), so `list` stops probing after the first keystore failure
and marks the remaining entries `not probed` rather than waiting again per
service. Unlock the keystore and re-run to get a full table.

Services that do not read the keystore decline `set` instead of writing an
entry nothing would use:

```
$ nono credential set anthropic
nono: service 'anthropic' reads environment variable ANTHROPIC_API_KEY, not the keystore.
      Export it before running nono: export ANTHROPIC_API_KEY=<secret>
      To store a keystore entry anyway, re-run with --account <name>.
```

The secret is only ever read from stdin — never from an argument, where it
would be visible to every process on the host and land in shell history. When
stdin is not a terminal the prompt is skipped and the secret is read as a plain
line, so setup stays scriptable:

```bash
printf %s "$OPENAI_API_KEY" | nono credential set openai
```

## Provisioning Without nono

`nono credential set` is the supported way to write a keystore entry. Reach for
the platform tools only when nono cannot run at the point the secret is
provisioned — an image build, a config-management step — or when you are
debugging an entry that was written by hand and is not being picked up.

Run `nono credential list` to see which account a service reads before writing
anything; the table it prints is the source of truth, and `--json` gives the
keystore service and account verbatim. A built-in route that omits
`credential_key` in `network-policy.json` reads the keystore service `nono`
under a username matching the service name.
...
The `keyring` crate looks entries up by the `service`, `username`, and `target`
attributes. You must use these exact attribute names — `account` instead of
`username`, or a missing `target default`, produces an entry that exists in the
keyring but that nono cannot find:
...
### Services that do not read the keystore

Some built-in routes read a host environment variable rather than the keystore,
so no keystore entry is consulted at all. `nono credential list` marks these,
and `nono credential set` declines them with the variable name. To store a
keystore entry for a service that reads elsewhere — because a profile will
point `credential_key` at it — pass `nono credential set <service> --account
<name>`.
