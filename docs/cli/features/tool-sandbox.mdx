...
An effective command sandbox with `network.allow_domain` receives a dedicated loopback proxy and fresh proxy credential. Its allowlist does not inherit a broader outer-session allowlist, although session-level deny rules still apply. The child sandbox may connect only to its selected proxy, so changing proxy environment variables, setting `NO_PROXY`, or opening a direct socket does not bypass the command policy unless the sandbox separately grants raw TCP access.

Proxy credentials granted by that effective sandbox remain available through the dedicated proxy. Credential reverse routes may reach their configured upstreams under `endpoint_policy`, without adding those upstreams to the general domain allowlist. Direct and ordinary forward-proxy requests still require `network.allow_domain` permission.

A sandbox that grants a proxy credential without `network.allow_domain` also receives a dedicated proxy with an empty general domain allowlist. Only its explicitly granted credential routes are available.
...
A command-scoped domain policy needs an active nono proxy, such as one activated by top-level `network.allow_domain`. A command-scoped proxy credential activates the proxy by itself. If the selected command sandbox requires a scoped proxy but none is available, the command fails closed at launch.
...
`sandbox` and `from.session` are alternative direct-session forms and cannot both be present. Chained access requires both the caller's `can_use` entry and the callee's `from.<caller>` policy; a missing caller edge does not fall back to the direct sandbox. Direct, caller-specific, and intercept domain allowlists are independent and are never merged. The caller's own sandbox controls the caller, not its child.
