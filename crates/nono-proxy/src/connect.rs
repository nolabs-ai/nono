    ...
    match decision {
        NetworkApprovalDecision::Granted(scope) => {
            ...
            let resolved = if scope == ApprovalScope::Once {
                let runtime_check = ctx.runtime_filter.check_host(&host, port).await?;
                if !runtime_check.result.is_allowed() || runtime_check.resolved_addrs.is_empty() {
                    let reason = match runtime_check.result {
                        nono::net_filter::FilterResult::DenyLinkLocal { .. } => {
                            "Host resolved to link-local address after approval"
                        }
                        _ => "Host still denied after approval",
                    };
                    audit::log_denied(
                        ctx.audit_log,
                        audit::ProxyMode::Connect,
