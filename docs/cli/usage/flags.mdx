...
Enable credential injection for a named service via the reverse proxy. The service must be either a preset service (`openai`, `anthropic`, `gemini`, `google-ai`, `github`, `gitlab`) or defined as a custom credential in your profile. Credentials are loaded from the system keyring under the `nono` service name, from 1Password when `credential_key` is an `op://` URI, from Apple Passwords on macOS when `credential_key` is an `apple-password://` URI, from a file when `credential_key` is a `file://` URI, or from the host environment when `credential_key` is an `env://` URI.

See [built-in credential sources](/cli/features/credential-injection#storing-credentials) for each service's configured source. `--credential openai` uses keyring account `openai` under service `nono`; `--env-credential openai_api_key` loads a different account and injects its secret directly as `OPENAI_API_KEY` in the child environment.
...
Extend the selected profile with an additional base profile for one invocation. Accepts a profile name, registry reference, or file path. Repeatable; bases are merged left-to-right, then any `extends` already in the selected profile is resolved, and the selected profile remains the final override layer.

```bash
nono run --profile my-agent \
  --extends linux-host-compat \
  --extends ./project-overlay.json \
  -- my-agent
```

Relative file paths resolve from the current directory, like `--profile`. Use `./` to distinguish a local path from a registry reference such as `namespace/name`. File bases support JSON comments and resolve their own inheritance, including named sibling profiles, before merging. The profile JSON's `extends` field still accepts only names and registry references.

The merge order is equivalent to temporarily prepending those bases to the profile JSON's `extends` field. It requires `--profile` and conflicts with `--config`. Inherited filesystem and network grants can widen sandbox permissions, so treat each base profile as explicit policy input.
