...
- **`environment` section omitted entirely**: all variables pass through (backward compatible).
- **`environment` present, `allow_vars` not written** (e.g. only `deny_vars` or `set_vars`): all variables pass through, minus any `deny_vars`.
- **`allow_vars: []` explicitly written**: restricts the environment to only nono-injected credentials — no inherited variables are passed.
...
`*` may appear anywhere in a pattern — leading, trailing, or in the middle — and matches any run of characters (including none):

```json
{
  "environment": {
    "allow_vars": ["PATH", "HOME", "AWS_*", "*_TOKEN", "AWS_*_SESSION", "MYAPP_*"]
  }
}
```

This passes through `AWS_REGION` and any variable starting with `AWS_` or `MYAPP_`, anything ending in `_TOKEN`, and anything matching `AWS_*_SESSION` (e.g. `AWS_ROLE_SESSION`) — while blocking everything else. A pattern with no `*` at all must match the full variable name exactly: `"PATH"` matches `PATH` but not `PATH_EXTRA`.

A bare `"*"` matches all variables (equivalent to not setting the `environment` section at all).

By default, matching is case-sensitive. Set `case_insensitive_vars: true` to match patterns and variable names case-insensitively (ASCII only):
...
This passes through `GH_TOKEN`, `Gh_Token`, etc. `case_insensitive_vars` is sticky across `extends`: once a base profile sets it to `true`, a child profile cannot silently turn it back off.

Malformed patterns — an empty string, or a pattern containing a NUL byte — are rejected at profile load time, not silently ignored.
...
Wildcards work the same as in `allow_vars` — `*` anywhere in the pattern:

```json
{
  "environment": {
    "deny_vars": ["GITHUB_*", "*_TOKEN", "*SECRET*"]
  }
}
```

This strips any var starting with `GITHUB_`, anything ending in `_TOKEN`, and anything containing `SECRET` anywhere in its name.
...
- **Default-allow**: Without the `environment` section, or with `environment` present but `allow_vars` not written, all variables pass through (no regression)
- **Explicit empty allow-list**: `"allow_vars": []` written explicitly passes zero inherited variables — only nono-injected credentials reach the child
...
- **Glob patterns are literal, not regex**: `*` is the only wildcard, matching any run of characters; a pattern with no `*` requires an exact full-name match, reducing risk of accidental over-permission
- **Malformed patterns fail closed**: an empty pattern or one containing a NUL byte is rejected at profile load time rather than silently ignored at runtime
