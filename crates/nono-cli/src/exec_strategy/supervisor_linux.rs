...
pub(super) fn handle_seccomp_notification(
    ...
    // Provide GPU comm fast-path: NVIDIA driver 570+ names threads by writing to
    // /proc/<tgid>/task/<tid>/comm via openat. Landlock covers /proc/self/task
...
mod proc_task_comm_tests {
    use super::is_proc_task_comm_for_tgid;
    use std::path::Path;
    ...
    fn rejects_non_comm_paths() {
        ...
        assert!(!is_proc_task_comm_for_tgid(
            Path::new("/proc/12345/task/12345/maps"),
            12345
        ));
        assert!(!is_proc_task_comm_for_tgid(
...
/// tgid component equals `expected_tgid`. The tgid check prevents a compromised
/// agent from writing thread names into other processes via the fast-path.
