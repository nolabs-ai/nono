...
`--no-tools` only affects the human-readable listing. `--json` always carries the same data per session as `tool_sandbox_active` and the full `command_policy_summary` object, and `nono audit show --json` reports the underlying `command_policy_events`, one entry per mediated decision — except for a denial repeated identically, which is recorded at the 1st, 2nd, 4th, 8th and so on rather than on every attempt, each such record naming the attempt it stands for.
...
- That rollup names a bounded number of distinct commands. A session that mediates more than the bound drops the per-command counts for the overflow and sets `truncated`; the session totals stay correct and every decision is still counted.
- A denial repeated identically is not appended on every attempt. The first is recorded, then the 2nd, 4th, 8th and so on, each annotated with the attempt number it stands for, so a process retrying one blocked operation contributes a logarithmic number of records instead of burying every other event. Counts are unaffected -- a suppressed denial is still counted against its command in the rollup -- so the log is a complete record of *distinct* denials and a sampled record of repeated ones, and the last checkpoint bounds how many attempts occurred.
