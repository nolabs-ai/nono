...
| Resource limits | Memory cap (cgroup v2, Linux) | Yes (cgroups) |
...
nono can cap memory with `--memory` (a cgroup v2 hard ceiling on Linux; see [Resource limits](/cli/features/resource-limits)), so a runaway agent cannot exhaust host RAM. It does not yet limit CPU or I/O — for those, reach for a container.
