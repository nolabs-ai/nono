...
//! It covers file capability, network, and command-policy launch requests.
...
/// [`ApprovalRequest::Capability`] variant. Command-policy launch uses the
/// [`ApprovalRequest::Command`] variant, built directly in the command-mediation shim
...
pub enum ApprovalRequest {
    ...
    /// A command-policy launch approval request (from the `Approve` intercept action).
    Command {
        /// Unique identifier for this request
        request_id: String,
        ...
        args: Vec<String>,
        /// Command-policy caller identity (e.g. `"session"` or a chained command name)
        caller: String,
