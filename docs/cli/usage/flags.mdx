...
- **Mid-session**: A running session (including a standalone `nono proxy`) picks up a certificate another session renewed with no prompt at all, and renews itself only if nothing newer is available. If renewal is declined or fails, the session keeps using its current certificate and retries later; a headless session (no interactive session to prompt) stops attempting to renew for the rest of the run but keeps polling to adopt a certificate another, interactive session renews
- **Headless launch**: Starting with no interactive session available still reuses a stored certificate that hasn't expired yet; only an already-expired certificate falls back to an untrusted ephemeral CA
