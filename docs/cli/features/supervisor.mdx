...
Sandbox denial details rely on access to macOS unified logs. If log collection
fails, nono warns that denial details may be incomplete; macOS can restrict log
access for standard (non-admin) accounts. The warning does not change sandbox
enforcement or the command's exit status. `--no-diagnostics` suppresses the human
warning, while `--diagnostics-json` includes it in the session diagnostics.
