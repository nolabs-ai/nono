...
mod tests {
    ...
    fn test_generate_profile_keeps_keychain_mach_deny_for_exact_file_grant() {
        // The library is policy-free: a grant supplies file access, it does not
        // decide that keychain access is authorized. Only a client-supplied
        // platform rule reopens the Mach services.
        let mut caps = CapabilitySet::new();
    ...
    fn test_generate_profile_keychain_mach_allow_rule_lands_after_deny() {
        // How a client authorizes keychain access: a platform rule. Seatbelt takes
        // the last matching rule, so the allow must be emitted after the deny.
        let mut caps = CapabilitySet::new();
