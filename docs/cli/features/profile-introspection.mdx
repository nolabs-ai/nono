...
# Emit fully resolved effective rules as JSON (same resolution as nono run)
nono profile show claude-code --format manifest
```
...
| `--format manifest` | Emit the fully resolved effective rules as JSON (same resolution as `nono run`) |
...
`--format manifest` emits the fully resolved effective rules as JSON, using the same extends + group resolution that `nono run` uses. This is useful for policy-as-code pipelines, diffing resolved rules across environments, or feeding the output into other tooling:
