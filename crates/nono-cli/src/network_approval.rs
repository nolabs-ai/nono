...
use nono::{
    ApprovalBackend, ApprovalDecision, ApprovalScope, NetworkApprovalDecision,
...
/// Entry in the pending map for deduplicating concurrent approval requests
/// for the same host. When the first request completes, the entry is removed
/// and waiters re-check the runtime filter (which now reflects the decision).
enum PendingEntry {
...
impl ConfigWriter {
    ...
    /// Append a host to the `network.deny_domain` array in the profile file.
    ///
    /// Reads the current file, adds the host (skipping duplicates), and
    /// writes it back. Creates the file with an `extends` reference if it
    /// doesn't exist yet.
    ...
    pub fn persist_deny(&self, host: &str) -> Result<()> {
        self.persist_domain("deny_domain", host)
    }

    /// Shared implementation for persisting a domain to a network config array.
    ///
    /// `field` is either `"allow_domain"` or `"deny_domain"`.
...
impl NetworkApprovalBackend {
    ...
    pub async fn request_network_approval_async(
        ...
        // Defense-in-depth: check runtime filter before showing dialog.
        // If host is already allowed, return immediately without prompting.
        // If host is explicitly denied, return Denied immediately.
        {
            let result = self.runtime_filter.check_host(&host, &[]);
            if result.is_allowed() {
                tracing::info!(
                    "Host {host} already in runtime allowlist — granting without dialog"
                );
                return NetworkApprovalDecision::Granted(ApprovalScope::Session);
            }
            if matches!(result, nono::net_filter::FilterResult::DenyHost { .. }) {
                tracing::info!("Host {host} in runtime denylist — denying without dialog");
                return NetworkApprovalDecision::Denied {
                    reason: "Host is denied".to_string(),
                };
            }
        }
        ...
        let (notify, is_first) = {
            ...
            match pending.remove(&host) {
                Some(PendingEntry::InProgress(n)) => {
                    tracing::info!("Host {host} has pending approval — waiting on existing dialog");
                    pending.insert(host.clone(), PendingEntry::InProgress(n.clone()));
                    (n, false)
                }
                None => {
        ...
        if !is_first {
            ...
            // The first request has completed (or we timed out). Re-check
            // the runtime filter: if the first request approved Session/Always,
            // the host is now in the allowlist. If denied Session/Always, it's
            // in the denylist. For Once decisions, the filter is unchanged.
            let result = self.runtime_filter.check_host(&host, &[]);
            if result.is_allowed() {
                return NetworkApprovalDecision::Granted(ApprovalScope::Session);
            }
            return NetworkApprovalDecision::Denied {
                reason: "Concurrent approval completed or timed out".to_string(),
            };
        }
                ...
                match result {
                    NotificationResult::Approve(duration) => match duration {
                        ...
                        ApprovalDuration::Always => {
                            ...
                            if let Some(ref writer) = self.config_writer
                                && let Err(e) = writer.persist_host(&host)
                            {
                                tracing::warn!(
                    ...
                    NotificationResult::Deny(duration) => match duration {
                        ...
                        ApprovalDuration::Always => {
                            ...
                            if let Some(ref writer) = self.config_writer
                                && let Err(e) = writer.persist_deny(&host)
                            {
                                tracing::warn!(
        ...
        // Remove the InProgress entry and notify all waiters. Waiters that
        // wake up will find no entry, re-enter the function, and hit the
        // early-return filter check — which now reflects the updated state
        // (host added to allowlist/denylist for Session/Always decisions).
        // For Once decisions, a new dialog may be shown; this is correct
        // since Once means single-use.
        {
            let mut pending = self.pending.lock().expect("pending lock poisoned");
            pending.remove(&host);
        }
...
impl ApprovalBackend for NetworkApprovalBackend {
    ...
    fn request_network_approval(
        ...
        if !is_first {
            ...
            // The first request has completed (or we timed out). Re-check
            // the runtime filter for updated state.
            let result = self.runtime_filter.check_host(&host, &[]);
            if result.is_allowed() {
                return Ok(NetworkApprovalDecision::Granted(ApprovalScope::Session));
            }
...
mod tests {
    ...
    #[test]
    fn test_persist_deny_creates_deny_domain() {
        let dir = tempfile::tempdir().expect("tempdir");
        let profile_path = dir.path().join("test-profile.json");
        std::fs::write(&profile_path, r#"{"meta":{"name":"test"}}"#).expect("write");
        let writer = ConfigWriter::new(profile_path.to_str().expect("valid utf-8 path"));
        ...
        let parsed: serde_json::Value = serde_json::from_str(&content).expect("parse");
        assert_eq!(parsed["meta"]["name"], "test");
        assert_eq!(parsed["network"]["deny_domain"][0], "evil.com");
    }
    ...
    fn test_persist_deny_no_duplicate() {
        let dir = tempfile::tempdir().expect("tempdir");
        let profile_path = dir.path().join("test-profile.json");
        std::fs::write(&profile_path, r#"{"network":{"deny_domain":["evil.com"]}}"#)
            .expect("write");
        let writer = ConfigWriter::new(profile_path.to_str().expect("valid utf-8 path"));
        ...
        let content = std::fs::read_to_string(&profile_path).expect("read");
        let parsed: serde_json::Value = serde_json::from_str(&content).expect("parse");
        let count = parsed["network"]["deny_domain"]
            .as_array()
            .expect("array")
            .iter()
    ...
    fn test_deny_always_adds_to_deny_filter_and_persists() {
        ...
        std::fs::write(
            &profile_path,
            r#"{"network":{"deny_domain":["existing-evil.com"]}}"#,
        )
        .expect("write");
        let writer = ConfigWriter::new(profile_path.to_str().expect("valid utf-8 path"));
        ...
        let content = std::fs::read_to_string(&profile_path).expect("read");
        let parsed: serde_json::Value = serde_json::from_str(&content).expect("parse");
        let domains = parsed["network"]["deny_domain"].as_array().expect("array");
        assert!(
            domains
                .iter()
