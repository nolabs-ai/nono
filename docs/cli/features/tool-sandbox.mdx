...
If a referenced variable is unset, or the expanded path does not exist at
launch, the credential is *unavailable*: nono logs one warning naming
...
If the expanded path exists but is not a socket, or nono cannot safely inspect
the path, that is a policy error rather than an availability condition, and the
individual command is denied.
...
| `local-socket credential unavailable; commands declaring it run without it` (warning) | The variable backing a local-socket credential is unset, or the path it expands to does not exist. Commands that select it still run, without the socket grant and without the mapped `env_var`. Start an agent if the command needs the credential. |
...
| `Tool Sandbox credential '<name>' is unavailable: failed to resolve/stat <path>` | nono could not safely determine the local-socket credential's state. The command is denied rather than treating an enforcement or filesystem error as ordinary absence. |
